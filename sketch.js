...
function setup() {
  ...
  player.speed = 5;
  level = new Level;
  edge.push(createSprite((displayWidth/2)-960-200,displayHeight/2,400,1080));
  edge.push(createSprite(displayWidth/2,(displayHeight/2)-540-200,1920,400));
  edge.push(createSprite((displayWidth/2)+960+200,displayHeight/2,400,1080));
  edge.push(createSprite(displayWidth/2,(displayHeight/2)+540+200,1920,400));
  edge_corner.push(createSprite((displayWidth/2)-960-200,(displayHeight/2)-540-200,400,400));
  edge_corner.push(createSprite((displayWidth/2)+960+200,(displayHeight/2)-540-200,400,400));
  edge_corner.push(createSprite((displayWidth/2)+960+200,(displayHeight/2)+540+200,400,400));
  edge_corner.push(createSprite((displayWidth/2)-960-200,(displayHeight/2)+540+200,400,400));
  for(var i = 0; i < 4; i++){
...
function draw() {
  ...
  if(gamestate === "menu"){
    ...
    for(var i = 0; i < 5; i++){
      ...
      switch(i){
        case 0:
          image(easy,0,i*100,666,96);
          break;
        case 1:
          image(medium,0,i*100,666,96);
          break;
        case 2:
          image(hard,0,i*100,666,96);
          break;
        case 3:
          image(insane,0,i*100,666,96);
          break;
        case 4:
          image(impossible,0,i*100,666,96);
          break;
      }
    }
    pop();
    if(mouseIsPressed){
      if(mouseX >= 785 && mouseX <= 1280){
        if(mouseY >= 625 && mouseY < 860){
          switch(difficulty){
            case 0:
              lives = 60;
              lifechance = 50;
      ...
      if(mouseX >= 1500 && mouseX <= 1900){
        if(mouseY >= 605 && mouseY < 855){
          rpg = 1;
          //gamestate = "rules";
          change = true;
          changeto = "rules";
      ...
      if(mouseX >= 95 && mouseX <= 761){
        if(mouseY >= 520 && mouseY <= 620){
          if(difficulty !== 0){
            selection.play();
          }
          difficulty = 0;
        }else if(mouseY >= 620 && mouseY <= 720){
          if(difficulty !== 1){
            selection.play();
          }
          difficulty = 1;
        }else if(mouseY >= 720 && mouseY <= 820){
          if(difficulty !== 2){
            selection.play();
          }
          difficulty = 2;
        }else if(mouseY >= 820 && mouseY <= 920){
          if(difficulty !== 3){
            selection.play();
          }
          difficulty = 3;
        }else if(mouseY >= 920 && mouseY <= 1025){
          if(difficulty !== 4){
            selection.play();
          }
          difficulty = 4;
    ...
    if(mouseIsPressed){
      if(mouseX > 1010 && mouseX < 1430){
        if(mouseY > 865 && mouseY < 1035){
          mouseIsPressed = false;
          rpg -= 1;
          mouseIsPressed = false;
        }
      }else if(mouseX > 1460 && mouseX < 1880){
        if(mouseY > 860 && mouseY < 1035){
          mouseIsPressed = false;
          rpg += 1;
          mouseIsPressed = false;
        }
    ...
    camera.zoom = 5 * displayHeight/1080;
    //background(a); 
    level.play();
  }
  if(change){
