        ...
        if(gamestate === "play"){
            if(lvl === 1){
                ...
                if(create){
                    ...
                    robot.push(createSprite(displayWidth/2 + 4,displayHeight/2 + 2.5,10,10)); //robot[1] FINAL BOSS
                ...
                if(start === 0){
                    rbt[0].m = false;    
                    if(player.t > 0 && player.x !== displayWidth/2 - 20 && player.y !== displayHeight/2 - 11.25){
                        player.t -= 34;
                    }
                    if(player.t <= 0){
                        player.x = displayWidth/2 - 20;
                        player.y = displayHeight/2 - 11.25;
                    }
                    if(player.t <= 221 && player.x === displayWidth/2 - 20 && player.y === displayHeight/2 - 11.25){
                        player.t += 34;
                    }
                    if(player.t >= 255 && player.x === displayWidth/2 - 20 && player.y === displayHeight/2 - 11.25 && mouseIsPressed || flag === 1){
                        start = 1;
                        rbt[0].m = true;
                    }
                    if(player.x === displayWidth/2 - 20 && player.y === displayHeight/2 - 11.25){
                        rbt[0].move();
                    }
                }
                if(start === 1){
                    ...
                    if(!won && !go){
                        if(opaque){
                            ...
                            textStyle(BOLD);
                            textFont("OCR A");
                            textSize(15*1920/displayWidth);
                            strokeWeight(1);
                            fill("purple");
                            stroke(255,255,255,200);
                            text("DISAPPEAR IN: " + roundoff(appearin,1), camera.position.x, camera.position.y - displayHeight/(camera.zoom * 2) + 18); 
                        }else{
                            textAlign(CENTER);
                            textStyle(BOLD);
                            textFont("OCR A");
                            ...
                            fill("purple");
                            stroke(255,255,255,200);
                            text("APPEAR IN: " + roundoff(appearin-10+endbreaks,1), camera.position.x, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
                        }
                    }
                    if(endbreaks >= 5){
                        won = true;
                    ...
                    if(rbt[0].dashin >= -0.5 && !go){
                        ...
                        if(rbt[0].dashin > 0.1){
                            fill("purple");
                            stroke(255,255,255,200);
                            text("\nDASH IN: " + roundoff(rbt[0].dashin,1), camera.position.x, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
                        }else{
                            fill("#B80F0A");
                            stroke(255,255,255,200);
                            text("\nDASH!", camera.position.x, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
                        }
                    }
            ...
            strokeWeight(1);
            if(lives > 0){
                text("lives: "+lives+"\nlevel: "+lvl, camera.position.x-displayWidth/(camera.zoom*2) + 7, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
                textAlign(RIGHT);
                text("FPS: "+round(frameRate()), camera.position.x+displayWidth/(camera.zoom*2) - 7, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
            }else{
                text("lives: none :(\nlevel: "+lvl, camera.position.x-displayWidth/(camera.zoom*2) + 7, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
                textAlign(RIGHT);
                text("FPS: "+round(frameRate()), camera.position.x+displayWidth/(camera.zoom*2) - 7, camera.position.y - displayHeight/(camera.zoom * 2) + 18);
